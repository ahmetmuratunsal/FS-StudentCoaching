...
import { getOneUser } from "../../redux/userSlice/userActions";
...
const UserDetail = () => {
  ...
  const { isError, isLoading, user } = useSelector((store) => store.user);
        ...
        <div className="p-4 md:p-12 text-center lg:text-left">
          ...
          {user?.user?.lesson && (
            <p className="pt-4 text-base font-bold flex items-center justify-center lg:justify-start">
              ...
              <span className="ml-2">
                {" "}
                {changeCategoryName(user?.user?.lesson)}
              </span>
            </p>
          )}
          ...
          <p className="pt-2 text-gray-600 text-xs lg:text-sm flex items-center justify-center lg:justify-start">
            ...
            Bulunduğu Şehir :{" "}
            <span className="mx-1"> {changeCityName(user?.user?.city)}</span>
          </p>

          <p className="pt-2 text-gray-600 text-xs lg:text-sm flex items-center gap-3 justify-center lg:justify-start">
            <VscQuestion className="text-xl text-green-700" />
            {user?.user?.isStudent ? "Sorduğu" : "Cevapladığı"} Soru Sayısı :{" "}
            <span className="mx-1 font-bold text-base text-red-500">
              {" "}
              {user?.user?.isStudent
                ? user?.user?.questionCount
                : user?.user?.answeredCount}
            </span>
          </p>
          {user?.user?.isStudent && (
            <p className="pt-2 text-gray-600 text-xs lg:text-sm flex items-center justify-center lg:justify-start">
              ...
              <span className="mx-1 text-red-500 text-base font-bold">
                {" "}
                {user?.user?.studentReviewCount}
              </span>
            </p>
          )}

          {!user?.user?.isStudent && (
            <p className="pt-8 text-sm">{user?.user?.bio}</p>
          )}
